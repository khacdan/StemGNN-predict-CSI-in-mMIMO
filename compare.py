...
decoder = Decoder()
decoder.to(device)
torch.save(decoder, f"decoder_{encoded_dim}")
...
x_test = torch.load(f"D:/CMa/{cr}/target_{mode}_{cr}_{kmph}_{hori}.pt").float()
x_hat  = torch.load(f"D:/CMa/{cr}/predict_{mode}_{cr}_{kmph}_{hori}.pt").float()
...
torch.save(torch.tensor(h_test[:,0,:,:]+1j*h_test[:,1,:,:]), f"D:/CSIP/{cr}_{kmph}/H_test_{cr}_{kmph}_{mode}_{hori}")
torch.save(torch.tensor(h_hat[:,0,:,:]+1j*h_hat[:,1,:,:]), f"D:/CSIP/{cr}_{kmph}/H_hat_{cr}_{kmph}_{mode}_{hori}")
